...
if(isset($_POST['add_product'])){
   ...
   if(mysqli_num_rows($select_product_name) > 0){
      $message[] = 'Produto com este mesmo nome já adicionado!';
   }else{
...
if(isset($_GET['delete'])){
   ...
   $delete_image_query = mysqli_query($conn, "SELECT image FROM `products` WHERE NameID = '$delete_id'") or die('query failed');
   $fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
   unlink('uploaded_img/'.$fetch_delete_image['image']);
   mysqli_query($conn, "DELETE FROM `products` WHERE NameID = '$delete_id'") or die('query failed');
...
<section class="show-products">
   <div class="box-container">
      ...
      <div class="box">
         ...
         <a href="admin_products.php?update=<?php echo $fetch_products['ProductID']; ?>" class="btn">Subir</a>
         <a href="admin_products.php?delete=<?php echo $fetch_products['ProductID']; ?>" class="btn" onclick="return confirm('delete this product?');">Excluir</a>
      </div>
      <?php
         }
      }else{
         echo '<p class="empty">Não há produtos adicionado de momento</p>';
      }
      ?>
   </div>
